        ...
        if (!isEmpty(result.data)) {
          ...
          if (result.data.rejectedSlot) {
            if (!isEmpty(result.data.employer_cancel_note)) {
              $('#scheduleSlotBookValidationErrorsBox').removeClass('d-none').html(result.data.employer_fullName + Lang.get('messages.job_stage.cancel_your_selected_slot') + '<br>' + '<b>Reason</b>:- ' + result.data.employer_cancel_note);
