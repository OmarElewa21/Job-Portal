...
$(document).on('click', '.edit-btn', function (event) {
  ...
  ajaxCallInProgress();
  var slotId = $(event.currentTarget).attr('data-id');
  $.ajax({
...
$(document).on('click', '.cancel-slot', function (e) {
  e.preventDefault();
  var cancelSlotNote = $(this).parent('.choose-slot-textarea').find('textarea.cancel-slot-notes').val().trim();
...
var resetScheduleSlotIndex = function resetScheduleSlotIndex() {
  var data = {
    'uniqueId': uniqueId
  };
  var index = 1;
...
var timePickerFunction = function timePickerFunction(selector) {
...
var dateTimePickerFunction = function dateTimePickerFunction(selector) {
  $(document.getElementById(selector)).datetimepicker(DatetimepickerDefaults({
    format: 'DD-MM-YYYY',
    useCurrent: true,
    sideBySide: true,
